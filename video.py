...
class Get_Danmaku_heuristic(object):
    ...
    def get_total_danmaku(self):
        ...
        for i in range(page_len):
            ...
            # print(data)
            name = self.bvid + '_' + str(info["pages"][i]["cid"]) +'Heuristic'+ '.csv'
...
#https://api.bilibili.com/x/v2/dm/web/view?type=1&oid=477163684&pid=677989591
def get_danmaku_view(page_id: int, bvid: str = None, aid: int = None, verify: utils.Verify = None):
    if verify is None:
        verify = utils.Verify()
    if not (aid or bvid):
    ...
    if resp.ok:
        resp_data = resp.content
        user_data=dmView_pb2.DmWebViewReply()
        user_data.ParseFromString(resp_data)
        UserData=json.loads(MessageToJson(user_data))
        return UserData
    else:
        raise exceptions.NetworkException(resp.status_code)
    ...
    def parse_bdoc(url, params=None, cookies=None, headers=None):
        ...
        if req.ok:
            ...
            if content_type == 'application/json':
                ...
                if data==b'\x10\x01':
                    raise exceptions.BilibiliApiException('视频弹幕已关闭')
                user_data=dm_pb2.DmSegMobileReply()
                user_data.ParseFromString(data)
                json_data=json.loads(MessageToJson(user_data))
                def microSecondsTo(microseconds):
                    s,ms=divmod(microseconds,1000)
                    m,s=divmod(s,60)
                    h,m=divmod(m,60)
...
class Get_Danmaku(object):
    ...
    def get_latest_danmaku(self, page_id):
        self.page_id = page_id
        latest_danmakus = get_danmaku(bvid=self.bvid, page_id=self.page_id, verify=self.verify)
    ...
    def get_history_danmaku(self, page_id, date):
        self.page_id = page_id
        self.date = datetime.datetime.strptime(date, '%Y-%m-%d').date()
        history_danmaku = get_danmaku(bvid=self.bvid, page_id=self.page_id, verify=self.verify, date=self.date)
        self.history_danmaku = [str(x) for x in history_danmaku]
        return self.history_danmaku
    ...
    def get_total_danmaku(self):
        ...
        for i in range(page_len):
            ...
            data = pd.DataFrame([i.split(',', 1) for i in self.output],
                                columns=['send_time', 'text'])
            # print(data)
            name = self.bvid + '_' + str(info["pages"][i]["cid"]) +'Exact'+ '.csv'
            data.to_csv(name, index=False, sep=',')
